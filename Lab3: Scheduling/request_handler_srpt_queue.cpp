...
//TODO fill in
RequestHandlerSrptQueue :: RequestHandlerSrptQueue(Json :: Value& config) : RequestHandler(config)
{
...
void RequestHandlerSrptQueue :: handleRequest(Request* req, ListenerEnd<Request*>* completionCallback)
{
  ...
  //when there is more than one request in the system,update its remaining work
  if (num_req > 1) 
  {
  ...
  if (queue_request.size() == 1)
   ...
   //checking the condition for preemption if a new request comes
   else if( req->getRemainingWork() < c_req->getRemainingWork() )
  {
...
uint64_t RequestHandlerSrptQueue :: request_size_extractor()
  {
      Request* R;
      uint64_t sizes = 0;
      std :: multimap <uint64_t,Request*>::iterator it;
...
void RequestHandlerSrptQueue :: UpdateReq(Request* req)
{
    std :: multimap <uint64_t,Request*>::iterator it;
    for(it = queue_request.begin(); it != queue_request.end(); it++)
    {
